...
while($arrNews = $this->db->fetch_array($tableNews))
{
	...
	echo "<a class=\"news-title\" href=\"".(isset($_REQUEST["p"])&&trim($_REQUEST["p"])!=""?$_REQUEST["p"]."-":"").($this->GetParam("SEO_URLS")==1?$M_SEO_NEWS."-".$arrNews["id"]."-".$this->format_str($arrNews["title"]).".html":"index.php?mod=news&id=".$arrNews["id"].($MULTI_LANGUAGE_SITE?"&lang=".$this->lang:""))."\">".stripslashes(strip_tags($arrNews["title"]))."</a>
	<span class=\"sub-text\">
