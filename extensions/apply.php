		...
		else
		{
			$show_page_form = false;
				$iGuestID = $database->SQLInsert
					...
					$database->SQLInsert
						("apply",
							array("employer_user","date","posting_id","jobseeker","guest","guest_id","message"),
							array($arrPosting["employer"],time(),$posting_id,$user_email,1,$iGuestID,get_param("message"))
						);
				...
				{	
					...
					$website->send_mail($oPosting["employer"], "".$M_NEW_JOBSEEKER_APPLIED.": \"".strip_tags($arrPosting["title"])."\"", $message, $headers);
				}
	...
	else
	{
					...
					else
					{
						...
						{	
							...
							$website->send_mail($oPosting["employer"], "".$M_NEW_JOBSEEKER_APPLIED.": \"".strip_tags($oPosting["title"])."\"", $message, $headers);
						...
						?>
						<script>
						DeleteSavedListing(<?php echo $posting_id;?>)
						</script>
...
if($show_page_form && isset($_COOKIE["AuthJ"]))
{
	...
	if($userExists&&md5($arrJ["password"])==$password)
	{
		...
		$proceed_application = true;
	
		///if charging the job seekers
		if($website->GetParam("CHARGE_THE_JOBSEEKERS")==1)
		{
			if($arrJ["package"]==0&&$arrJ["credits"]<$website->params[705])
			{
				echo "<h4>".$M_NOT_ENOUGH_CREDITS_TO_APPLY."</h4>";
				echo "<br/><br/>";
				echo "<h4><a class=\"underline-link\" href=\"JOBSEEKERS/index.php?category=home&action=credits\">".$M_CLICK_PURCHASE_MORE."</a></h4>";
				$proceed_application = false;
			}
		
		}
		///end if charging the job seekers
		
		
		if($proceed_application)
		{
	...
	?>
				...
				while($userFile = $database->fetch_array($userFiles))
				{
					...
					echo "
						<td width=20>
							<input type=checkbox ".(isset($_POST["Ids"])&&in_array($userFile["file_id"], $_POST["Ids"])?"checked":"")." name=Ids[] value=\"".$userFile["file_id"]."\">
						</td>
					";
