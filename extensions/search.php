...
if(isset($_REQUEST["search"])&&$_REQUEST["search"]==1)
{
		...
		if(get_param("job_title") != "")
		{
			...
			$strSearchQuery .= "AND (".$DBprefix."employers.company LIKE '%".$database->escape_string($search_title)."%' OR ".$DBprefix."jobs.title LIKE '%".$database->escape_string($search_title)."%' OR ".$DBprefix."jobs.message LIKE '%".$database->escape_string($search_title)."%')";
			$indeed_query  .= get_param("job_title")." ";
		}
		
		if(get_param("bonuses") != "")
		{
			$strSearchQuery .= "AND ".$DBprefix."jobs.bonuses=1 ";
		}
		
		if(get_param("salary_from") != "")
		{
			$strSearchQuery .= "AND ".$DBprefix."jobs.salary>=".get_param("salary_from")." ";
		}
		
		if(get_param("salary_to") != "")
		{
			$strSearchQuery .= "AND ".$DBprefix."jobs.salary<=".get_param("salary_to")." ";
		}
		
		if(get_param("salary_type") != "")
		{
			$strSearchQuery .= "AND ".$DBprefix."jobs.salary_type=".get_param("salary_type")." ";
		}
		...
		("
			...
			".$DBprefix."jobs.salary_type,
			".$DBprefix."jobs.job_type,
			".$DBprefix."jobs.bonuses,
			".$DBprefix."jobs.applications,
			".$DBprefix."jobs.job_category,
...
if(isset($_REQUEST["location"]))
{
	...
	$arrCustomTitle=$database->DataArray("custom_titles","region='".$location."'");

	if(trim($arrCustomTitle["title"])!="")
	{
		$website->Title(stripslashes($arrCustomTitle["title"]));
	}
	else
	{
	...
	if(trim($arrCustomTitle["description"])!="")
	{
		$website->MetaDescription(stripslashes($arrCustomTitle["description"]));
	}
	else
	{
	...
	if(trim($arrCustomTitle["keywords"])!="")
	{
		$website->MetaKeywords($website->format_keywords(stripslashes($arrCustomTitle["keywords"])));
	}
	else
	{
	...
	)
	{
		...
		if(substr_count($category,".")>0)
		{
			...
			for($i=0;$i<sizeof($category_items)-1;$i++)
			{
				...
				$strLink = $website->category_link($current_id,$current_category);
		...
		$arrCustomTitle=$database->DataArray("custom_titles","job_category='".$category."'");
...
{
	if(trim($job["job_category"])!="")
	{
		$cat_items=explode(".",$job["job_category"]);
		
		if(!isset($filter_categories[$cat_items[0]]))
		{
			$filter_categories[$cat_items[0]]=1;
		}
		else
		{
			$filter_categories[$cat_items[0]]++;
		}
	}
	
	if(trim($job["job_type"])!="")
	{
		if(!isset($filter_types[$job["job_type"]]))
		{
			$filter_types[$job["job_type"]]=1;
		}
		else
		{
			$filter_types[$job["job_type"]]++;
		}
	}
	
	if(trim($job["region"])!="")
	{
		$location_items=explode(".",$job["region"]);
		
		if(!isset($filter_locations[$location_items[0]]))
		{
			$filter_locations[$location_items[0]]=1;
		}
		else
		{
			$filter_locations[$location_items[0]]++;
		}
	}
...
if($website->GetParam("FEEDS_USAGE")!=0&&!isset($_REQUEST["company"]))
{
	...
	$feed_page=$num;
	$total_search_pages=ceil($iTotResults/$website->GetParam("RESULTS_PER_PAGE"));
	if($feed_page>0)
	{
		$feed_page-=$total_search_pages;
	}

	$feeds=get_feeds($indeed_query,(isset($selected_location_name)?$selected_location_name:$indeed_location),$str_i_co,$feed_page,20,get_param("zip"),get_param("zip_distance"));
	$iJCounter_indeed=0;
...
if($website->GetParam("FEEDS_USAGE")!=0&&$iJCounter_indeed>0&&$website->feed_jobs==0)
{
...
else
{
	...
	$total_pages=ceil(($iTotResults+$website->feed_jobs)/$website->GetParam("RESULTS_PER_PAGE"));
	...
	if($total_pages > 1)
	{
		...
		$page_start_number=$num-1;
		$page_end_number=$num+3;
		if($page_start_number<1)
		{
			$page_end_number+=abs($page_start_number)+1;
			$page_start_number=1;
		}
		if($page_end_number>$total_pages)
		{
			$page_start_number-=($page_end_number>$total_pages);
			$page_end_number=$total_pages;
		}
		
		echo "<li ".($num==0?"class=\"active\"":"")."><a class=\"pagination-link\" href=\"index.php?".$strSearchString."num=1\">1</a></li>";
		
		
		if($page_start_number>2)
		{
			echo "<li><a>...</a></li>";
		}
		
		for($i=$page_start_number;$i<=$page_end_number;$i++)
		{
			if($i==0||$i==1||$i==$total_pages) continue;
			echo "<li ".($i == ($num+1)?"class=\"active\"":"")."><a class=\"pagination-link\" href=\"index.php?".$strSearchString."num=".$i."\">".$i."</a></li>";
		}
		...
		echo "<li ".($num==($total_pages-1)?"class=\"active\"":"")."><a class=\"pagination-link\" href=\"index.php?".$strSearchString."num=".$total_pages."\">".$total_pages."</a></li>";
				
		if(($num+1)<$total_pages)
		{
			echo "<li><a href=\"index.php?".$strSearchString."num=".($num+2)."\"> > </a></li>";
		}
		...
		for($i= $iStartNumber-2 ;$i<=ceil($iTotResults/$website->GetParam("RESULTS_PER_PAGE"));$i++)
		{
			if($inCounter>=5)
			{
				break;
		...
		if($num<ceil($iTotResults/$website->GetParam("RESULTS_PER_PAGE")))
		{
...
$website->filters["categories"]=$filter_categories;
$website->filters["locations"]=$filter_locations;
$website->filters["types"]=$filter_types;
?>
</div>
</div>
<?php
