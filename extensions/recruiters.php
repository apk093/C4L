...
$table_recruiters = $database->DataTable("employers","WHERE active=1 ".$search_query." ORDER BY featured_employer DESC");
...
$iTotResults=$database->num_rows($table_recruiters);
$i_counter=0;

while($current_recruiter = $database->fetch_array($table_recruiters))
{
	if($i_counter>=$num*$RESULTS_PER_PAGE&&$i_counter<($num+1)*$RESULTS_PER_PAGE)
	{
		$str_job_count=$database->SQLCount("jobs","WHERE ".($website->GetParam("ADS_EXPIRE")!=-1?" expires>".time()." AND ":"")." employer='".$current_recruiter["username"]."'");
					
	//	if($str_job_count==0) continue;
			...
			<div class="panel panel-default">
				...
				<div class="panel-body padding-10">
						<div class="row">
							...
							if($database->num_rows($latest_listings)>0)
							{							
							?>		
