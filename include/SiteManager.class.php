...
class SiteManager
{
	...
	function LoadTemplate($template_id)
	{
		...
		if(!$this->isMainAdminPanel && $this->admin_settings["custom_color"]!="")
		{
		...
		if(!$this->isAdminPanel)
		{
			...
			if($this->db->num_rows($tableBanners)>0)
			{
				...
				while($arrBanner = mysqli_fetch_array($tableBanners))
				{
					...
					$areaHTML .= "<img alt=\"\" width=\"".$arrBannerArea["width"]."\" src=\"".($arrBannerArea["height"]<=75&&$arrBannerArea["width"]<=100?"thumbnails":"uploaded_images")."/".$arrBanner["image_id"].".jpg\" style=\"margin-bottom:5px;margin-right:5px\"/>";		
	...
	function LanguagesMenu($page)
	{
		...
		$tableLanguages=$database->DataTable("languages","WHERE active=1 ORDER BY code='".strtoupper($this->lang)."' DESC");
		
		$arrDefaultLanguage=$database->fetch_array($tableLanguages);
		
		$strResult.='<a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false"><img src="images/flags/'.strtoupper($arrDefaultLanguage["code"]).'.gif" class="language-flag-icon"/> '.stripslashes($arrDefaultLanguage["name"]).' <span class="caret"></span></a>';
               
		$strResult.=' <ul class="dropdown-menu language-select">';
			...
			while($arrLanguages=$database->fetch_array($tableLanguages))
			{
				...
				if(!$bFirst)	
				{
					$strResult.="<li role=\"separator\" class=\"divider\"></li>";
				}
				
				$strResult.=
				"
					<li><a href=\"".$str_link."\"><img src=\"images/flags/".strtoupper($arrLanguages["code"]).".gif\" class=\"language-flag-icon\"/> <span  class=\"language-select-link\">".stripslashes($arrLanguages["name"])."</span></a></li>
	...
	function GenerateMenu()
	{
		...
		foreach($this->arrPages as $arrPage)
		{
			...
			if($this->DefaultPageId == $arrPage[0]) 
			{
				...
				if(isset($_REQUEST["page"])&&$_REQUEST["page"]==$this->DefaultPage)
				{
					$this->is_default_page=true;
				}
			}
	...
	function MetaDescription($meta_description)
	{
		...
		(
			"<site description/>",
			strip_tags(stripslashes(str_replace('&nbsp;',' ',str_replace('"','',$meta_description)))),
			$this->TemplateHTML
		);
	}
	...
	function MetaKeywords($meta_keywords)
	{
		$pattern = '/[^\pL ]+/';
		$replacement = '';
		$strTitle = mb_ereg_replace($pattern, $replacement, str_replace('"','',$meta_keywords));

		$meta_keywords = str_replace("&nbsp;"," ",trim($strTitle));
		$meta_keywords = str_replace("...","",$meta_keywords);
		$meta_keywords = str_replace(" ",",",trim($meta_keywords));
	...
	function GetParam($param_name)
	{
		switch($param_name)
		{
			...
			case "CHARGE_THE_JOBSEEKERS":
				return $this->params[898];
				break;
				
		
			case "PRICE_LISTING_CREDITS":
				return $this->params[699];
				break;
				
			case "FREE_CREDITS_NEW_USERS":
				return $this->params[706];
				break;
			...
			case "BLACK_LISTED_WORDS":
				return $this->params[614];
				break;	
			...
			case "STRIPE_PUBLISHABLE_KEY":
				return $this->params[470];
				break;
				
			case "STRIPE_SECRET_KEY":
				return $this->params[471];
				break;
			...
			case "USE_SMTP":
				return $this->params[148];
				break;
				
			case "SMTP_SERVER":
				return $this->params[149];
				break;
				
			case "SMTP_USER":
				return $this->params[150];
				break;	
				
			case "SMTP_PASSWORD":
				return $this->params[151];
				break;	
				
			case "SMTP_SECURE":
				return $this->params[152];
				break;
				
			case "SMTP_PORT":
				return $this->params[153];
				break;
				
			case "SALARY_RANGE":
				return $this->params[154];
				break;
...
//		   die("-".$loc[$arr_digits[0]]."-");
	...
	function send_mail($email_address, $email_subject, $email_text, $headers="")
	{
		
		if($this->GetParam("USE_SMTP")==0)
		{
			$headers  = "From: \"".$this->GetParam("SYSTEM_EMAIL_FROM")."\"<".$this->GetParam("SYSTEM_EMAIL_ADDRESS").">\n";
			$headers .= "MIME-Version: 1.0\r\n";
			$headers .= "Content-Type: text/html; charset=utf-8\r\n";

			if(mail($email_address, $email_subject, $email_text, $headers))
			{
				return true;
			}
			else
		...
		{
			if(file_exists("include/class.phpmailer.php"))
			{
			...
			$mail = new PHPMailer();
			$mail->IsSMTP();
			$mail->Host=$this->GetParam("SMTP_SERVER");
			$mail->SMTPAuth=true;
			
			if($this->GetParam("SMTP_SECURE")!=""&&$this->GetParam("SMTP_SECURE")!="none")
			{
				$mail->SMTPSecure = $this->GetParam("SMTP_SECURE");
			}
			
			if($this->GetParam("SMTP_PORT")!="")
			{
				$mail->Port=$this->GetParam("SMTP_PORT");
			}
			
			$mail->Username   = $this->GetParam("SMTP_USER");
			$mail->Password   = $this->GetParam("SMTP_PASSWORD");      
			
			$mail->isHTML(true);

			
			$mail->SetFrom( $this->GetParam("SMTP_USER"), $this->GetParam("SYSTEM_EMAIL_FROM"));
