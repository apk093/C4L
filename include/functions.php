...
{
	global $website,$M_SHORTLIST,$M_SHORTLISTED,$M_SAVED,$M_JOB_DETAILS,$M_APPLICATIONS,$M_POSTED_ON;
	...
	global $M_PER,$M_HOUR,$M_WEEK,$M_MONTH,$M_YEAR,$M_INCLUDING_BONUSES;
	$salary_types=array("",$M_HOUR,$M_WEEK,$M_MONTH,$M_YEAR);
	
	if($job["salary"]!=""&&$job["salary"]!="0")
	{
		echo '<li>'.$M_SALARY.': <strong>'.$website->GetParam("WEBSITE_CURRENCY").stripslashes($job["salary"]).' '.$M_PER.' '.$salary_types[$job["salary_type"]].' '.($job["bonuses"]==1?", ".$M_INCLUDING_BONUSES:"").'</strong></li>';
	}
	if($job["region"]!="")
	{
	...
	if(isset($_COOKIE["saved_listings"]) && strpos($_COOKIE["saved_listings"], $job["id"].",") !== false)
	{
		...
		{
			echo '<a class="job-details-link underline-link r-margin-15" href="javascript:DeleteSavedListing('.$job["id"].')" id="save_'.$job["id"].'">'.$M_SHORTLISTED.'</a>';
		}
	}
	else
	{
		echo '<a class="job-details-link underline-link r-margin-15" href="javascript:SaveListing('.$job["id"].')" id="save_'.$job["id"].'">'.$M_SHORTLIST.'</a>';
	}
	...
	echo '</div>
		...
		$strCompanyLink=$website->company_link($job["employer_id"],$job["company"]);
		
		echo '
		<div class="logo-wrap">
			<a href="'.$strCompanyLink.'">';
...
function show_feed_job($job)
{
	...
	echo '<div class="results-job-description">'.$website->text_words(stripslashes(strip_tags($job["description"])),30).'</div>';
	echo '<div class="clearfix"></div>';
...
function get_feeds($query,$location,$country,$i_page,$page_size,$zip="",$zip_radius="")
{
	...
	$black_list_query="";
	if(trim($website->GetParam("BLACK_LISTED_WORDS"))!="")
	{
		$arr_words=explode(",",$website->GetParam("BLACK_LISTED_WORDS"));
		
		foreach($arr_words as $str_word)
		{
			$black_list_query.="+-".trim($str_word);
		}
	}
	...
	{
		$jt="";
		if(isset($_REQUEST["job_type"])&&$_REQUEST["job_type"]=="1") $jt="fulltime";
		if(isset($_REQUEST["job_type"])&&$_REQUEST["job_type"]=="2") $jt="contract";
		if(isset($_REQUEST["job_type"])&&$_REQUEST["job_type"]=="3") $jt="parttime";
		if(isset($_REQUEST["job_type"])&&$_REQUEST["job_type"]=="4") $jt="internship";
		if(isset($_REQUEST["job_type"])&&$_REQUEST["job_type"]=="5") $jt="temporary";
		
		if($zip!=""&&$zip_radius!="")
		{
			$indeed_url = "http://api.indeed.com/ads/apisearch?v=2&publisher=".$website->GetParam("INDEED_PUBLISHER_ID")."&q=".urlencode($query).$black_list_query."&l=".$zip."&sort=date&radius=".$zip_radius."&st=&jt=".$jt."&start=".$i_start_1."&limit=".$limit_results_1."&fromage=".(isset($_REQUEST["posting_date"])?$_REQUEST["posting_date"]:"%20")."&filter=&latlong=1&co=".$country."&chnl=&userip=".$client_ip."&useragent=".urlencode($_SERVER['HTTP_USER_AGENT']);
		}
		else
		{
			$indeed_url = "http://api.indeed.com/ads/apisearch?v=2&publisher=".$website->GetParam("INDEED_PUBLISHER_ID")."&q=".urlencode($query).$black_list_query."&l=".urlencode($location)."&sort=date&radius=&st=&jt=".$jt."&start=".$i_start_1."&limit=".$limit_results_1."&fromage=".(isset($_REQUEST["posting_date"])?$_REQUEST["posting_date"]:"%20")."&filter=&latlong=1&co=".$country."&chnl=&userip=".$client_ip."&useragent=".urlencode($_SERVER['HTTP_USER_AGENT']);
		}
	
		$xml = simplexml_load_file($indeed_url);
		
		$website->feed_jobs=$xml->totalresults;
