...
<?php		
	...
	foreach($arrJobFields as $arrJobField)
	{
		...
		$strSpecialHiddenFieldsToAdd.= "<td >".str_show($arrJobField[0], true).":</td>";	
		
		$strSpecialHiddenFieldsToAdd.= "<td >";
		
		if(trim($arrJobField[2]) != "")
		{
				$strSpecialHiddenFieldsToAdd.= "<select  name=\"pfield".$iFCounter."\" style=\"width:".$_REQUEST["select-width"]."px\">";
		...
		{
				$strSpecialHiddenFieldsToAdd.= "<input value=\"".(isset($arrPropFields[$arrJobField[0]])?$arrPropFields[$arrJobField[0]]:"")."\" type=text name=\"pfield".$iFCounter."\" style=\"width:".$_REQUEST["select-width"]."px\">";
		}
...
{
	array_push($_REQUEST["arrNames2"],"salary_type");
	array_push($_REQUEST["arrValues2"],$_POST["salary_type"]);
}

if(isset($_POST["bonuses"]))
{
	array_push($_REQUEST["arrNames2"],"bonuses");
	array_push($_REQUEST["arrValues2"],"1");
}
...
if($insertID > 0)	
{
	if(!isset($doNotAdd))
	{
		...
		if($website->GetParam("ENABLE_EMAIL_NOTIFICATIONS"))
		{
			...
			$tableRules = $database->DataTable("rules","WHERE job_category='".$_POST["job_category"]."' ");
			
			while($arrRule = $database->fetch_array($tableRules))
			{
				if($arrRule["region"] != ""&&isset($_POST["region"]))
				{
					if($arrRule["region"] != $_POST["region"]) continue;
				}
		...
		{	
			...
			{
				echo "<br/><hr/><br/>";
				echo "<img src='images/bank-transfer.png' align='left' style='margin-right:30px;'/><h4>".$M_AMOUNT.": <strong>".$website->GetParam("CURRENCY").number_format($website->GetParam("PRICE_JOB"), 2, '.', '')."</strong><br/><br/><br/>".nl2br(stripslashes($website->GetParam("BANK_ACCOUNT")))."</h4>";
			}
