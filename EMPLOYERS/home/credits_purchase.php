...
<?php
if($website->GetParam("STRIPE_PUBLISHABLE_KEY")!="")
{
	require_once('../include/stripe/Stripe.php');

	$stripe = array
	(
	  "secret_key"      => $website->GetParam("STRIPE_SECRET_KEY"),
	  "publishable_key" => $website->GetParam("STRIPE_PUBLISHABLE_KEY")
	);
...
<h3><?php echo $M_CREDIT_PURCHASE;?></h3>
		...
		<br>
		<?php 
		if(isset($_REQUEST["ProceedStripe"]))
		{
		  ...
		  $token  = $_POST['stripeToken'];
			...
			(
				...
				array(time(),$AuthUserName,$number_credits,"stripe",$price,"1")
			);
		...
		?>
			...
			<?php
				...
				?>
					...
					<input type="hidden" name="ProceedStripe" value="1">
					<input type="hidden" name="category" value="<?php echo $category;?>">
					<input type="hidden" name="action" value="<?php echo $_POST["action"];?>">

					<input type="hidden" name="amount" value="<?php echo number_format($price*100, 0, '.', '');?>">
					<input type="hidden" name="number_credits" id="number_credits" value="<?php echo $number_credits;?>" >
				
					 <script src="https://button.stripe.com/v1/button.js" class="stripe-button"
							  data-key="<?php echo $website->GetParam("STRIPE_PUBLISHABLE_KEY"); ?>"
							  data-amount="<?php echo number_format($price*100, 0, '.', '');?>" data-description="<?php echo $DOMAIN_NAME." ".$number_credits." ".$M_CREDITS;?>"></script>
					</form>
		...
		?>
				...
				<input type="radio" <?php if(!$bFirstChecked) echo "checked";?> name="payment_method" value="stripe">
				Payment with credit card
			
				<img src="images/card_logos.gif" alt="" border="0">
