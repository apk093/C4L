...
function AddNewForm
	...
	if(isset($_REQUEST["SpecialProcessAddForm"]))
	{
		...
		for($i=0;$i<sizeof($arrNames);$i++)
		{
			...
			if($arrTypes[$i]=="pdf_field")
			{
				global $_FILES;
				$pdf_value="";
				if(isset($_FILES["pdf"]['tmp_name'])&&trim($_FILES["pdf"]['tmp_name'])!="")
				{
					$random_id =mt_rand(100000,9999999);
					move_uploaded_file($_FILES["pdf"]['tmp_name'], "../user_files/" . $random_id.".pdf");
					$pdf_value=$random_id;
				}
				else
				if(isset($_POST["pdf_url"])&&trim($_POST["pdf_url"])!="")
				{
					$pdf_value=str_ireplace("http://","",str_ireplace("https://","",$_POST["url"]));
				}
			
				array_push($arrInsertName,"pdf");
				array_push($arrValues,$pdf_value);
			}
			else
			if($arrTypes[$i]=="file")
			{
				$iFileId=insert_image($strName);
				array_push($arrInsertName,($strName=="image_id"?$strName:"logo"));
	...
	else
	{
		...
		for($i=0;$i<sizeof($arrTexts);$i++)
		{
			...
			if(strstr($arrTypes[$i],"salary_field"))
			{
				global $M_YEAR,$M_MONTH,$M_WEEK,$M_HOUR,$M_PER,$M_BONUSES;
				
				echo "<input type=\"number\" step=\"0.01\" name=\"salary\" style=\"width:150px;\"/>";
				
				echo ' <select name="salary_type">
						<option value="1">'.$M_PER.' '.$M_HOUR.'</option>
						<option value="2">'.$M_PER.' '.$M_WEEK.'</option>
						<option value="3" selected>'.$M_PER.' '.$M_MONTH.'</option>
						<option value="4">'.$M_PER.' '.$M_YEAR.'</option>
				
						</select> ';
				
				echo '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="checkbox" name="bonuses" value="1"> '.$M_BONUSES;
			}
			else
...
<?php				
			...
			if(strstr($arrTypes[$i],"pdf_field"))
			{
				echo "<input ".(isset($_REQUEST["select-width"])?"style=\"width:".$_REQUEST["select-width"]."px\"":"")."  type=\"file\" name=\"pdf\" size=\"10\"/>";
				
				global $M_OR_ENTER_URL;
				
				echo " ".$M_OR_ENTER_URL.": <input type=\"text\" name=\"pdf_url\" placeholder=\"http://\" size=\"35\" style=\"margin-bottom:7px;margin-top:7px\"/>";
			}
			else
			...
			?>
				...
				$_REQUEST["M_ALL"]=$M_PLEASE_SELECT;
					...
					echo '<select '.(isset($_REQUEST["select-width"])?"style=\"width:".$_REQUEST["select-width"]."px\"":"").' '.($field_name=="category_1"?'required':'').' onChange="sub_loc_select(this.value,\''.$field_name.'\',\''.$M_PLEASE_SELECT.'\',\'../include/suggest'.(isset($_REQUEST["action"])&&$_REQUEST["action"]=="add_course"?'_course':'').'_category.php?lang='.$lang.'\')" type="text" class="sub-loc-select" id="'.$field_name.'_0" name="'.$field_name.'_0">';
					echo '<option value="">'.$_REQUEST["M_ALL"].'</option>';
...
function particularCases($columnName,$myArray)
{
	...
	if($columnName=="ShowRenew")
	{
		...
		{
			return "<td><a href='index.php?category=".$_REQUEST["category"]."&action=".$_REQUEST["action"]."&renew=1&id=".$myArray['id'].(isset($_REQUEST["PageNumber"])?"&PageNumber=".$_REQUEST["PageNumber"]:"")."' ><img src=\"../images/renew.png\" width=\"22\" height=\"22\" alt=\"\" border=\"0\"/></a></td>";
		}
	}
	else
	...
	{
		if($myArray['featured']=="1")
		{
			return "<td align=\"center\"><a href=\"index.php?category=".$_REQUEST["category"]."&action=".$_REQUEST["action"]."&featured=0&id=".$myArray['id']."\"><img border=\"0\" src=\"../images/active_".$myArray['featured'].".gif\"></a></td>";	
		}
		else
		{
...
function EditParams
	...
	while($row=$database->fetch_array($oTable))	
	{
		...
		if(strstr($arrTypes[$i],"textarea"))
		{
			...
			echo "<textarea  ".(isset($_REQUEST["select-width"])?"style=\"width:".$_REQUEST["select-width"]."px\"":"")." name=\"val".$row['id']."\" cols=$strCols rows=$strRows>".$row['value']."</textarea>";
		}
		else
		if(strstr($arrTypes[$i],"javascript~combobox")){
	...
	if(true)
	{
		...
		for($i=0;$i<sizeof($arrTexts);$i++)
		{
			...
			if($arrTexts[$i]!="")
			{
				echo 
				"
					<td  width=\"".(isset($MessageTDLength)?$MessageTDLength:"100")."\"/>
					".$arrTexts[$i]."
					</td>
				";
			}
			...
			if(strstr($arrTypes[$i],"address_field"))
			{

				global $M_PLEASE_ENTER_ADDRESS,$M_OR_ENTER,$M_LATITUDE,$M_LONGITUDE,$M_ADD_MAP;
			?>
		<script type="text/javascript" src="http://maps.google.com/maps/api/js?key=AIzaSyCYruCJM-kLZvueTK1rca8l0sYlzJOftKU&sensor=false"></script> 
      <script>
					  var geocoder;
					  var map;
					  function initialize() 
					  ...
					  function PreviewMap() 
					  {
						...
						{
							alert("<?php echo $M_PLEASE_ENTER_ADDRESS;?>");	
					...
					<br/>
						<input name="address"  <?php if(isset($_REQUEST["select-width"])) echo "style=\"width:".$_REQUEST["select-width"]."px\"";?> value="<?php echo stripslashes($oArray["address"]);?>" onkeyup="javascript:UserKeyUp()" id="address" type="text" placeholder="<?php echo $M_PLEASE_ENTER_ADDRESS;?>"/>
					
					
					<div style="margin-top:4px;margin-bottom:4px;" id="preview-map"><a class="underline-link" href="javascript:PreviewMap()"><?php echo $M_ADD_MAP;?></a></div>
					...
					<div style="display:none">
						...
						&nbsp;
						<?php echo $M_LATITUDE;?>:
						<input type="text" name="latitude" id="latitude"  style="width:60px" value="<?php echo stripslashes($oArray["latitude"]);?>">
						<span style="font-size:10px">e.g. 40.758224</span>
						&nbsp;&nbsp;
						<?php echo $M_LONGITUDE;?>:
						<input type="text" name="longitude" id="longitude" style="width:60px" value="<?php echo stripslashes($oArray["longitude"]);?>"> 
			...
			if(strstr($arrTypes[$i],"textarea")){
				...
				<textarea ".(isset($_REQUEST["select-width"])?"style=\"width:".$_REQUEST["select-width"]."px\"":"")." name=\"".$arrEditFields[$i]."\" id=\"".$arrEditFields[$i]."\" cols=\"".($strCols=="100%"?"20":$strCols)."\" rows=\"".$strRows."\" ".($strCols=="100%"?"style=\"width:100% !important\"":"").">".stripslashes($strVal)."</textarea>";
			}
			else
					...
					while (list($key, $val) = each($arrCategories)) 
					{
						...
						if(sizeof($arr_sub_cats)>0)
						{
							while (list($s_key, $s_val) = each($arr_sub_cats)) 
							{
								...
								$s_arr_sub_cats = get_sub_cats_level(2,$s_key,strtolower($lang));
								
								echo "<option value=\"".trim($s_key)."\" ".($strVal==trim($s_key)?"selected":"")." style=\"font-size:12px\">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;".trim($s_val)."</option>";
								
								if(sizeof($s_arr_sub_cats)>0)
								{
									while (list($s_s_key, $s_s_val) = each($s_arr_sub_cats)) 
									{
										echo "<option value=\"".trim($s_s_key)."\" ".($strVal==trim($s_s_key)?"selected":"")." style=\"font-size:11px\">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;".trim($s_s_val)."</option>";
										
										$s_s_arr_sub_cats = get_sub_cats_level(3,$s_s_key,strtolower($lang));
										if(sizeof($s_s_arr_sub_cats)>0)
										{
											while (list($s_s_s_key, $s_s_s_val) = each($s_s_arr_sub_cats)) 
											{
												echo "<option value=\"".trim($s_s_s_key)."\" ".($strVal==trim($s_s_s_key)?"selected":"")." style=\"font-size:10px\">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;".trim($s_s_s_val)."</option>";
											}	
										}
									}
								}
...
function get_sub_cats_level($level,$key,$lang)
{
	include("../categories/categories_array_".$lang.".php");
	$arr_result=array();
	if($level==2)
	{
		$key_items=explode(".",trim($key));
		$str_arr_name="l".$level;
		$c_array=$$str_arr_name;
		if(sizeof($key_items)==2&&isset($c_array[$key_items[0]][$key_items[1]]))
		{
		
			foreach($c_array[$key_items[0]][$key_items[1]] as $r_key=>$r_value)
			{
				$arr_result[$key.".".$r_key]=$r_value;
			}
		}
	}
	else 
	if($level==3)
	{
		...
		if(sizeof($key_items)==3&&isset($$str_arr_name)&&isset($$str_arr_name[$key_items[0]][$key_items[1]][$key_items[2]]))
		{
			foreach($$str_arr_name[$key_items[0]][$key_items[1]][$key_items[2]] as $r_key=>$r_value)
			{
				$arr_result[$key.".".$r_key]=$r_value;
			}
		}
