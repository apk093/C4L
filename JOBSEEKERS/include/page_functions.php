...
function AddNewForm
	...
	{
	global $_REQUEST,$lang,$database,$MessageTDLength,$website;
		...
		for($i=0;$i<sizeof($arrTexts);$i++)
		{
			...
			?>
				...
				$_REQUEST["M_ALL"]=$M_PLEASE_SELECT;
					...
					echo '<select '.(isset($_REQUEST["select-width"])?"style=\"width:".$_REQUEST["select-width"]."px\"":"").' '.($field_name=="category_1"?'required':'').' onChange="sub_loc_select(this.value,\''.$field_name.'\',\''.$M_PLEASE_SELECT.'\',\'../include/suggest_category.php?lang='.$lang.'\')" type="text" class="sub-loc-select" id="'.$field_name.'_0" name="'.$field_name.'_0">';
					echo '<option value="">'.$_REQUEST["M_ALL"].'</option>';
					if(file_exists("../categories/categories_array_".$website->lang.".php"))
					{
...
function particularCases($columnName,$myArray)
{
	...
	{   
		...
		{   
			global $M_OPEN;
			$strResult .= '   <a href="'.$file_show_link.'">'.$M_OPEN.'</a>   ';  
		}  
		$strResult .= "</td>";   
