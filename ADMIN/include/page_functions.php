	...
	)
{
	...
	if(trim($strCheckColumnName)!="")
	{
		echo "
			...
			<input type=\"submit\" value=\" ".$strCheckColumnName." \" class=\"btn btn-default btn-gradient pull-right\"/>
	...
	if(ceil($iTotalResults/$PageSize) > 1)
	{
		...
		if($PageNumber != 1)
		{
			echo "<li><a ".($ajax_call?"target=\"ajax-ifr\" onmousedown=\"javascript:LoadingIcon()\"":"")." class=\"small-nav-link\" href=\"".(isset($ajax_call)&&$ajax_call?"admin_page.php?ajax_load=1&":"index.php?").$strSearchString."PageNumber=1\"><<</a></li>";
			
			echo "<li><a ".($ajax_call?"target=\"ajax-ifr\" onmousedown=\"javascript:LoadingIcon()\"":"")." class=\"small-nav-link\" href=\"".(isset($ajax_call)&&$ajax_call?"admin_page.php?ajax_load=1&":"index.php?").$strSearchString."PageNumber=".($PageNumber)."\"><</a></li>";
		}
		...
		for($i= $iStartNumber ;$i<=ceil($iTotalResults/$PageSize);$i++)
		{
			...
			{
				echo "<li><a ".($ajax_call?"target=\"ajax-ifr\" onmousedown=\"javascript:LoadingIcon()\"":"")." class=\"small-nav-link\" href=\"".(isset($ajax_call)&&$ajax_call?"admin_page.php?ajax_load=1&":"index.php?").$strSearchString."PageNumber=".$i."\">".$i."</a><li>";
			}
		...
		{
			echo "<li><a ".($ajax_call?"target=\"ajax-ifr\" onmousedown=\"javascript:LoadingIcon()\"":"")." class=\"small-nav-link\" href=\"".(isset($ajax_call)&&$ajax_call?"admin_page.php?ajax_load=1&":"index.php?").$strSearchString."PageNumber=".($PageNumber+1)."\">></a></li>";
			
			echo "<li><a ".($ajax_call?"target=\"ajax-ifr\" onmousedown=\"javascript:LoadingIcon()\"":"")." class=\"small-nav-link\" href=\"".(isset($ajax_call)&&$ajax_call?"admin_page.php?ajax_load=1&":"index.php?").$strSearchString."PageNumber=".(ceil($iTotalResults/$PageSize))."\">>></a></li>";
		}
...
function particularCases($columnName,$myArray)
{
	...
	{  
		return "<td><a href='index.php?category=".$_REQUEST["category"]."&folder=jobseekers&page=cv&id=".$myArray['id']."' ><img height=\"22\" src=\"../images/cv.png\" alt=\"\"/></a></td>"; 
	}  
	else
	if($columnName=="ShowFiles")
	{  
		return "<td><a href='index.php?category=".$_REQUEST["category"]."&folder=jobseekers&page=files&id=".$myArray['id']."' ><img height=\"22\" src=\"../images/files.png\" alt=\"\"/></a></td>"; 
	}  
	else
	...
	{
		$html_content = "<td><img src=\"images/spacer.gif\" width=\"70\" height=\"1\">";
		
		$str_images_list="";
		
		if(isset($myArray['logo']))
		{
			$str_images_list=$myArray['logo'];
		}
		else
		{
			$str_images_list=$myArray['images'];
		}
		
		if($str_images_list!="")
		{
			
			$image_ids = explode(",",$str_images_list);
	...
	if(true)
	{
		...
		for($i=0;$i<sizeof($arrTexts);$i++)
		{
			...
			if($arrEditFields[$i]!="images"&&in_array($arrEditFields[$i],$arrMissedFields))
			{
				...
				{
					echo '<img src="../uploaded_images/'.$oArray[$arrEditFields[$i]].'.jpg" class="img-responsive"/>';
				}
				else
				{
			...
			if(strstr($arrTypes[$i],"textbox")){
				...
				list($strType,$strSize)=explode("_",$arrTypes[$i]);
					...
					echo "<input type=\"text\" value=\"".stripslashes($strVal)."\" name=\"".$arrEditFields[$i]."\" id=\"".$arrEditFields[$i]."\" size=\"".$strSize."\" ".(isset($_REQUEST["select-width"])?"style=\"width:".$_REQUEST["select-width"]."px\"":"")."/>";
			}
			else
			if(strstr($arrTypes[$i],"textarea")){
				list($strType,$strCols,$strRows)=explode("_",$arrTypes[$i]);
			...
			if(strstr($arrTypes[$i],"combobox_region"))
			{
				...
				foreach($arrCategories as $str_category)
				{
					$loc_items=explode(". ",$str_category);
					if(sizeof($loc_items)!=2) continue;
					$key=$loc_items[0];
					$value=$loc_items[1];
					$categories[trim($key)]=trim($value);
