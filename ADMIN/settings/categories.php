...
{
	if(file_exists('../categories/categories_'.strtolower($current_language["code"]).'.php'))
	{
...
if(isset($_REQUEST["ProceedSend"]))
{
	foreach($arr_languages as $str_lang)
	{
		...
		$handle = fopen('../categories/categories_'.$str_lang.'.php', 'w');
		...
		$handle = fopen('../categories/categories_array_'.$str_lang.'.php', 'w');
	...
	foreach($arr_languages as $str_lang)
	{
		...
		if(file_exists('../categories/categories_'.$str_lang.'.php'))
		{
			$lines = file('../categories/categories_'.$str_lang.'.php');
