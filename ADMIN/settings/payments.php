...
if(isset($_REQUEST["ProceedSave"]))
{
	...
	$charge_jobseekers=$_REQUEST["charge_jobseekers"];
	$website->ms_i($charge_jobseekers);
	$database->SetParameter(898, $charge_jobseekers);
}
else
{
	$charge_type=$website->GetParam("CHARGE_TYPE");
	$charge_jobseekers=$website->GetParam("CHARGE_THE_JOBSEEKERS");
}
...
<br/>
<br/>
<h4>Do you want to charge the job seekers?</h4>

<input class="margin-top-8" type="radio" <?php if($charge_jobseekers==0) echo "checked";?> name="charge_jobseekers" value="0"/>
No
&nbsp; 
<input class="margin-top-8" type="radio" <?php if($charge_jobseekers==1) echo "checked";?> name="charge_jobseekers" value="1"/>
Yes
