...
<?php
if(false&&$database->SQLCount("files","WHERE user='$jobseeker_user_name'","file_id") >= aParameter(816))
{
?>
<br><br><br>
	<span class="red_font">
	...
	{   
		...
		else  
		{
				...
				(
					...
					array($jobseeker_user_name,$file_name,time(),$file_type,$file_size,$iResult,get_param("file_description"),get_param("is_resume"))
				);	
...
if($database->SQLCount("files","WHERE user='".$jobseeker_user_name."'","file_id") == 0)
{
	echo "<br>
		<i>No files were found</i>
	";
}
else
{

$_REQUEST["hidden_fields"]=array();
$_REQUEST["hidden_fields"]["id"]=$id;

 RenderTable
 (
	"files",
	array("file_date","file_size","file_name","description","JobFileType"),
	array($DATE_MESSAGE,$SIZE,$NOM,$DESCRIPTION,$M_OPEN),
	"100%",
	"WHERE user='".$jobseeker_user_name."'",
	$EFFACER,
	"file_id",
	"index.php"
	);
